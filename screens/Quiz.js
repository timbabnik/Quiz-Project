...
const Quiz = ({navigation}) => {
    ...
    const [quizName, setQuizName] = useState("History");

    


    const validateAnswer = (selectedOption) => {
        let correct_option = data[questionsIndex]["correct_option"];
    ...
    const handleNext = () => {
        if(questionsIndex == data.length-1){
        ...
        return (
            <View>
                <View style={{flexDirection: "row", alignItems: "flex-end"}}>
                    ...
                    <Text style={{color: COLORS.white, fontSize: 18, opacity: 0.6}}>/ {data.length}</Text>
                </View>
                <Text style={{fontSize: 20, color: COLORS.white}}>{data[questionsIndex].question}</Text>
            </View>
        )
    }
    ...
    const renderOptions = () => {
        return (
            <View style={{marginTop: 20}}>
                {
                    data[questionsIndex].options.map((option) => (
                        <TouchableOpacity 
                ...
                >
                    <View style={{
                        ...
                        <View style={{
                            ...
                            <Text style={{fontSize: 30, fontWeight: "bold"}}> {score > (data.length/2) ? "Congratz" : "Ooopsss"} </Text>
