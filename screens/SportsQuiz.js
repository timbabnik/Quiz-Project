...
const Quiz = ({navigation}) => {
    ...
    const [quizName, setQuizName] = useState("NBA");


    const validateAnswer = (selectedOption) => {
        let correct_option = dataSport[questionsIndex]["correct_option"];
    ...
    const handleNext = () => {
        if(questionsIndex == dataSport.length-1){
        ...
        return (
            <View>
                <View style={{flexDirection: "row", alignItems: "flex-end"}}>
                    ...
                    <Text style={{color: COLORS.white, fontSize: 18, opacity: 0.6}}>/ {dataSport.length}</Text>
                </View>
                <Text style={{fontSize: 20, color: COLORS.white}}>{dataSport[questionsIndex].question}</Text>
            </View>
        )
    }
    ...
    const renderOptions = () => {
        return (
            <View style={{marginTop: 20}}>
                {
                    dataSport[questionsIndex].options.map((option) => (
                        <TouchableOpacity 
    ...
    return (
        <View style={{flex: 1, backgroundColor: "#4A2527"}}>
                ...
                >
                    <View style={{
                        ...
                        <View style={{
                            ...
                            <Text style={{fontSize: 30, fontWeight: "bold"}}> {score > (dataSport.length/2) ? "Congratz" : "Ooopsss"} </Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 30,
                                    color: score> (dataSport.length/2) ? COLORS.success : COLORS.error
                                }}>{score}</Text>
                                <Text style={{
                                    fontSize: 20, color: COLORS.black
                                }}>/ {dataSport.length}</Text>
                            </View>
